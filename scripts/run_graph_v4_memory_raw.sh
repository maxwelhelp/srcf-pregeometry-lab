...
python -u closure_graph_real_benchmark_v4_memory.py \
  ...
  --rel-mode raw \
  --use-memory --ablate-memory --ablate-tri \
  --memory-slots 16 --memory-beta 2.0 --memory-strength 0.20 \
  --eval-every 30 \
  --results-csv results/closure_graph_real_v4_memory_raw.csv \
  | tee closure_graph_real_v4_memory_raw.log
