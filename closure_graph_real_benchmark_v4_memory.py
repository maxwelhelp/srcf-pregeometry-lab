...
# -*- coding: utf-8 -*-
"""
ClosureGraphRealBenchmark v4 MEMORY
...
Цель:
  Проверить closure-layer на реальных графах честнее, чем v1:
    ...
    - optional Modern/Dense Hopfield style AttractorBank memory over relation states
...
Запуск быстрый:
  python -u closure_graph_real_benchmark_v4_memory.py --device cuda --amp fp16 --experiment per_graph --seeds 0 --steps 120

Запуск сильнее:
  python -u closure_graph_real_benchmark_v4_memory.py --device cuda --amp fp16 --experiment per_graph --seeds 0,1,2 --steps 150

Inductive:
  python -u closure_graph_real_benchmark_v4_memory.py --device cuda --amp fp16 --experiment inductive --train-graphs karate,lesmis --test-graphs florentine,davis --seeds 0,1,2
"""
...
class AttractorBank(nn.Module):
    """Modern/Dense-Hopfield style prototype memory for relation states.

    h: [B,N,N,D]
    returns:
      mem: [B,N,N,D] soft retrieval from learned slot prototypes
      stats: entropy/usage diagnostics when requested
    """
    ...
    def forward(self, h: torch.Tensor, return_stats: bool = False):
        # sim [B,N,N,K]
        sim = torch.einsum("bijd,kd->bijk", h, self.bank) * self.beta / math.sqrt(max(h.shape[-1], 1))
        attn = sim.softmax(dim=-1)
        mem = torch.einsum("bijk,kd->bijd", attn, self.bank)
        if not return_stats:
            return mem
        # Diagnostics: entropy low => confident slot retrieval; usage high => slots are used broadly.
        eps = 1e-9
        ent = (-(attn.clamp_min(eps) * attn.clamp_min(eps).log()).sum(dim=-1)).mean() / math.log(attn.shape[-1] + eps)
        usage = attn.mean(dim=(0, 1, 2))
        usage_ent = (-(usage.clamp_min(eps) * usage.clamp_min(eps).log()).sum()) / math.log(attn.shape[-1] + eps)
        max_slot = attn.max(dim=-1).values.mean()
        return mem, {"mem_entropy": float(ent.detach().cpu()), "mem_usage_entropy": float(usage_ent.detach().cpu()), "mem_max_slot": float(max_slot.detach().cpu())}


class ClosureLayer(nn.Module):
    def __init__(self, rel_dim: int, dim: int, hidden: int, use_triangle: bool = True, use_memory: bool = False, memory_slots: int = 16, memory_beta: float = 1.0, memory_strength: float = 0.25):
        super().__init__()
        ...
        self.memory = AttractorBank(dim, n_slots=memory_slots, beta=memory_beta) if use_memory else None
        # h,row,col,global,row_std,col_std,(triangle if enabled),(memory if enabled),memory_residual + 6 scalar graph features
        feat_mult = 6 + (1 if use_triangle else 0) + (2 if use_memory else 0)
        feat_dim = dim * feat_mult + 6
    ...
    def triangle_update(self, h: torch.Tensor) -> torch.Tensor:
        ...
        return tri
    def step_once(self, h: torch.Tensor, rel: torch.Tensor, return_mem_stats: bool = False):
        ...
        if self.use_memory and self.memory is not None:
            if return_mem_stats:
                mem, mem_stats = self.memory(h, return_stats=True)
            else:
                mem = self.memory(h, return_stats=False)
            # Give the step both the retrieved prototype and the residual to that prototype.
            pieces.append(mem)
            pieces.append(mem - h)
        a = self._ch(rel, 0)
        deg_i = a.sum(dim=2, keepdim=True).expand_as(a) / max(rel.shape[1]-1, 1)
        ...
        delta = self.step(x)
        gate = self.gate(x)
        out = self.ln(h + gate * delta)
        if mem is not None and self.memory_strength != 0.0:
            out = self.ln(out + self.memory_strength * torch.tanh(mem - h))
        if return_mem_stats:
            return out, mem_stats
        return out
    def forward(self, rel: torch.Tensor, iters: int = 5, return_curve: bool = False, return_h: bool = False):
        ...
        for _ in range(iters):
            if return_curve and self.use_memory and self.memory is not None and _ == iters - 1:
                h, mem_stats = self.step_once(h, rel, return_mem_stats=True)
            else:
                h = self.step_once(h, rel)
            if return_curve:
                curve.append(float((h - prev).pow(2).mean().detach().sqrt().cpu()))
            prev = h
        self.last_mem_stats = mem_stats
        out = self.read(h)
...
def make_models(args, device: torch.device) -> Dict[str, nn.Module]:
    ...
    models: Dict[str, nn.Module] = {
        ...
        "closure": ClosureLayer(rel_dim, args.dim, args.hidden, use_triangle=True, use_memory=args.use_memory, memory_slots=args.memory_slots, memory_beta=args.memory_beta, memory_strength=args.memory_strength).to(device),
    }
    if args.ablate_tri:
        models["closure_no_tri"] = ClosureLayer(rel_dim, args.dim, args.hidden, use_triangle=False, use_memory=args.use_memory, memory_slots=args.memory_slots, memory_beta=args.memory_beta, memory_strength=args.memory_strength).to(device)
    if args.ablate_memory:
        models["closure_no_mem"] = ClosureLayer(rel_dim, args.dim, args.hidden, use_triangle=True, use_memory=False).to(device)
    return models
            ...
            for split_name, gs in graphs_eval.items():
                for hard in (False, True):
                    ...
                    if eval_iters:
                        ...
                        iter_s = ""
                    extra_bits = []
                    if "closure_no_mem" in aucs:
                        mds = []
                        for task in ["edge", "path2", "comm"]:
                            a = aucs.get("closure", {}).get(task, float("nan"))
                            b = aucs.get("closure_no_mem", {}).get(task, float("nan"))
                            mds.append(f"{task}:{(a-b):+.3f}")
                        extra_bits.append("memΔ=" + "/".join(mds))
                    if "closure_no_tri" in aucs:
                        tds = []
                        for task in ["edge", "path2", "comm"]:
                            a = aucs.get("closure", {}).get(task, float("nan"))
                            b = aucs.get("closure_no_tri", {}).get(task, float("nan"))
                            tds.append(f"{task}:{(a-b):+.3f}")
                        extra_bits.append("triΔ=" + "/".join(tds))
                    extra_s = (" | " + " | ".join(extra_bits)) if extra_bits else ""
                    print(f"  {split_name}:{mode} " + " | ".join(parts) + iter_s + extra_s, flush=True)
...
def main() -> None:
    ...
    p.add_argument("--use-memory", action="store_true", help="enable AttractorBank memory inside the main closure layer")
    p.add_argument("--ablate-memory", action="store_true", help="also train closure_no_mem to measure memory contribution")
    p.add_argument("--memory-slots", type=int, default=16, help="number of learned attractor memory slots")
    p.add_argument("--memory-beta", type=float, default=2.0, help="softmax sharpness for memory retrieval")
    p.add_argument("--memory-strength", type=float, default=0.20, help="residual pull strength toward retrieved attractor memory")
    ...
    p.add_argument("--results-csv", default="closure_graph_real_v4_summary.csv")
    args = p.parse_args()

    print("ClosureGraphRealBenchmark v4 MEMORY — real graph closure layer", flush=True)
    ...
    print("baselines: raw, common-neighbors, jaccard, spectral, pair-MLP, deep-MLP, deep_big, GCN; closure uses triangle update; optional AttractorBank memory", flush=True)
    ...
    fieldnames = ["experiment","run","seed","split","corruption","task","model","auc","closure_auc","closure_delta_vs_best_nonclosure","best_nonclosure","steps","n","dim","hidden","iters","params","rel_mode","directed","use_memory","memory_slots","memory_beta","memory_strength"]
